...
const PROTOCOL_INFO: Record<Protocol, { label: string; description: string }> = {
...
const EXTENSION_INFO: ReadonlyArray<{
  id: WizardExtension;
  label: string;
  description: string;
}> = [
  { id: "te", label: "Traffic Engineering", description: "MPLS-TE extensions. Advertises bandwidth and delay." },
  { id: "mpls", label: "MPLS / LDP", description: "Label Distribution Protocol for MPLS forwarding plane." },
  { id: "sr", label: "Segment Routing", description: "Source-routed MPLS with SRGB label blocks." },
];
...
export function ProtocolSelection({ selected, rules, onSelect }: ProtocolSelectionProps) {
  ...
  return (
    <div className="wizard-protocol-list">
      {rules.available_protocols.map((protocol) => {
        const info = PROTOCOL_INFO[protocol];
        return (
        <button
          key={protocol}
          className={`wizard-protocol-btn ${selected === protocol ? "wizard-protocol-btn--selected" : ""}`}
                ...
                {protocol === "isis" ? (
                  <>
                    <TimerField label="Hello Interval" unit="s" value={routingTimers.isis_hello_interval}
                      onChange={(v) => onUpdateTimers({ isis_hello_interval: v })} min={1}
                    ...
                    <TimerField label="Hello Multiplier" value={routingTimers.isis_hello_multiplier}
                      onChange={(v) => onUpdateTimers({ isis_hello_multiplier: v })} min={1}
                    ...
                    <TimerField label="SPF Init Delay" unit="ms" value={routingTimers.spf_init_delay}
                      onChange={(v) => onUpdateTimers({ spf_init_delay: v })} min={0}
                    ...
                    <TimerField label="SPF Short Delay" unit="ms" value={routingTimers.spf_short_delay}
                      onChange={(v) => onUpdateTimers({ spf_short_delay: v })} min={0}
                    ...
                    <TimerField label="SPF Long Delay" unit="ms" value={routingTimers.spf_long_delay}
                      onChange={(v) => onUpdateTimers({ spf_long_delay: v })} min={0}
                    ...
                    <TimerField label="SPF Holddown" unit="ms" value={routingTimers.spf_holddown}
                      onChange={(v) => onUpdateTimers({ spf_holddown: v })} min={0}
                  ...
                  <>
                    <TimerField label="Hello Interval" unit="s" value={routingTimers.ospf_hello_interval}
                      onChange={(v) => onUpdateTimers({ ospf_hello_interval: v })} min={1}
                    ...
                    <TimerField label="Dead Interval" unit="s" value={routingTimers.ospf_dead_interval}
                      onChange={(v) => onUpdateTimers({ ospf_dead_interval: v })} min={1}
                      desc="Time without hellos before declaring neighbor dead."
                      range="Must exceed the hello interval." />
                    <TimerField label="SPF Delay" unit="ms" value={routingTimers.ospf_spf_delay}
                      onChange={(v) => onUpdateTimers({ ospf_spf_delay: v })} min={0}
                    ...
                    <TimerField label="SPF Initial Hold" unit="ms" value={routingTimers.ospf_spf_initial_hold}
                      onChange={(v) => onUpdateTimers({ ospf_spf_initial_hold: v })} min={0}
                    ...
                    <TimerField label="SPF Max Hold" unit="ms" value={routingTimers.ospf_spf_max_hold}
                      onChange={(v) => onUpdateTimers({ ospf_spf_max_hold: v })} min={0}
              ...
              {routingTimers.bfd && (
                <div className="wizard-timer-list" style={{ marginTop: 8 }}>
                  <TimerField label="Detect Multiplier" value={routingTimers.bfd_detect_multiplier}
                    onChange={(v) => onUpdateTimers({ bfd_detect_multiplier: v })} min={1}
                  ...
                  <TimerField label="RX Interval" unit="ms" value={routingTimers.bfd_rx_interval}
                    onChange={(v) => onUpdateTimers({ bfd_rx_interval: v })} min={1}
                  ...
                  <TimerField label="TX Interval" unit="ms" value={routingTimers.bfd_tx_interval}
                    onChange={(v) => onUpdateTimers({ bfd_tx_interval: v })} min={1}
