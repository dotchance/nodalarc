...
| **measurement/** | Measurement Infrastructure - FRR protocol adapters, convergence detection, probe daemon. Containerised but not deployed by default; the current session grammar has no activation field for it, and active development is deferred. | `python -m measurement.mi_main` |
