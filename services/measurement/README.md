...
- **Opt-in, not grammar-reachable today.** MI is not deployed by default, and the current session grammar has no field that activates it; wiring MI deployment into the session grammar is part of resuming MI development.
