"""Phase 5A: Verify checkpoint ConfigMap is written during Scheduler operation."""
...
def main():
    ...
    # Check checkpoint ConfigMap
    print("\n=== Checkpoint ConfigMap ===")
    cm = kubectl("get", "configmap", "nodalarc-scheduler-checkpoint", "-o", "yaml")
    if cm:
        print(cm)
        if "sim_time" in cm and "updated_at" in cm:
            print("\nPASS: Checkpoint ConfigMap written with sim_time and updated_at")
        else:
            print("\nFAIL: ConfigMap exists but missing expected fields")
    else:
        print("FAIL: ConfigMap not found")
