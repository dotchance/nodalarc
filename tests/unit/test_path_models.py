...
from nodalarc.models.path import PathHop, PathResult, PathQuery


def test_path_hop_valid():
    hop = PathHop(
        node_id="sat-P00S00", node_type="satellite",
        in_label=100, out_label=200, action="swap",
        out_interface="isl0", latency_to_next_ms=5.3,
    )
...
def test_path_result_unreachable():
    result = PathResult(
        ...
        reachable=False, unreachable_reason="no ingress rule",
    )
