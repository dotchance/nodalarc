...
class TestPropagate:
    def test_return_to_start_after_one_period(self):
        ...
        pos0, _, _ = earth_propagate_keplerian(elements, EPOCH, 0.0)
        pos1, _, _ = earth_propagate_keplerian(elements, EPOCH, period)
    ...
    def test_latitude_bounded_by_inclination(self):
        ...
        for step in range(0, int(period), 60):
            _, _, geo = earth_propagate_keplerian(elements, EPOCH, float(step))
    ...
    def test_different_raan_produces_different_positions(self):
        ...
        e2 = earth_elements_from_params(550.0, 53.0, 90.0, 0.0)

        pos1, _, _ = earth_propagate_keplerian(e1, EPOCH, 100.0)
        pos2, _, _ = earth_propagate_keplerian(e2, EPOCH, 100.0)
...
class TestGeodeticRoundTrip:
    ...
    def test_known_position(self):
        ...
        geo = earth_ecef_to_geodetic(pos)
