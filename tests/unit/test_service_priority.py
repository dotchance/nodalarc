...
from ome.ground_allocator import _compute_pair_score
...
def _score_and_sort(pairs_config):
    ...
    scored = []
    for gs_id, sat_id, elev, priority, _is_active, _min_elev in pairs_config:
...
class TestServicePriorityPreemption:
    ...
    def test_priority_is_selection_ranking_not_handover_stickiness(self):
        """Priority is a ranking component; incumbent stickiness is evaluated later."""
        sorted_pairs = _score_and_sort(
            [
                ("gs-gold", "sat-A", 30.0, 1, False, 25.0),
    ...
    def test_same_priority_uses_raw_selection_score_not_incumbent_boost(self):
        """Within the same tier, ranking uses raw selection scores only."""
        sorted_pairs = _score_and_sort(
            [
        ...
        assert sorted_pairs[0][2] == "gs-challenger"  # raw 44 > raw 40
